// pages/carNoMgr/carNoMgr.js
var app = getApp();
Page({
  data: {
    tempNo:null,
    whatsBottom: 1,//1--添加新车牌；2--确定；3--目前您只能添加3个车牌
    isEdit: [],
    addressList: null,
    editIndex: 0,
  ...
  //当显示为‘添加新车牌’，即取值为1时，点击之后，变为2--‘确定’
  //当显示为2--‘确定’时，点击后，向addressList中添加输入，然后需要根据addressList的长度，来判断显示1还是3。
  ...
  onUnload: function () {
    ...
    wx.request({
      url: app.globalData.globalURL + 'UpdateCarNo',
      data: {
        addressList: this.data.addressList,
  ...
  getCarNo: 　function (cb) {
    // 首先读取缓存，看缓存中是否存在addressList，若不存在，再去读取数据库
